...
echo "Please set the VNC password for user paul"
su - paul -c vncpasswd
...
vnc_user_config="/home/paul/.vnc/config"
...
touch /home/paul/.vnc/config
chmod 766 /home/paul/.vnc/config
echo "
geometry=1366x768
" > /home/paul/.vnc/config
restorecon -rv /home/paul/.vnc
...
firewall-cmd --permanent --zone=$currentZone --add-rich-rule='rule family="ipv4" source address="192.168.64.1/24" port port="'${ports[0]}'" protocol="tcp" accept'
firewall-cmd --reload

# Step 6: Echo the information that VNC client can connect to
red='\033[0;31m'
NC='\033[0m' # No Color
echo -e "${red}Display number for user paul is ${sessions[0]}${NC}"
